...
public class EntityGlitch extends EntityMob {
    ...
    protected ResourceLocation getLootTable() {
        return new ResourceLocation(DMLConstants.ModInfo.ID, "glitch");
    }
    ...
    public void onLivingUpdate() {
        if (world.isRemote) {
            ...
            for(int i = 0; i < 16; ++i) {
                DMLRelearned.proxy.spawnGlitchParticle(
                        world,
                        posX + (rand.nextDouble(-2.0D, 2.0D)) * (double) width,
                        posY + (rand.nextDouble(-0.5D, 0.5D)) * (double) height,
                        posZ + (rand.nextDouble(-2.0D, 2.0D)) * (double) width,
